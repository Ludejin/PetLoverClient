...
public final class ViewUtils {
     ...
     * 改变状态栏颜色
     *
     * @param statusColorRes 颜色的resource id
     */
    public static void compat(Activity activity, int statusColorRes) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ViewGroup contentView = (ViewGroup) activity.findViewById(android.R.id.content);
            if (contentView == null) {
                return;
            }
            int color = Color.parseColor("#C69986");
            if (statusColorRes != -1) {
                color = statusColorRes;
            }
    ...
    public static void expandViewTouchDelegate(final View view, final int size) {

        expandViewTouchDelegate(view, size, size, size, size);
    }
